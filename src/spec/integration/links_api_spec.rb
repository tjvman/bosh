...
describe 'links api', type: :integration do
  ...
  context 'when requesting for a list of links via links endpoint' do
    ...
    context 'when a link consumer id is specified' do
      context 'when there are links with that consumer id' do
        let(:provider_1) do
          {
            'name' => 'provider',
            'release' => 'bosh-release',
        ...
        let(:jobs) { [provider_1, consumer_1, consumer_2] }
        ...
        it 'returns the links with the specified id' do
          actual_response = get_json('/links', 'consumer_id=2')
